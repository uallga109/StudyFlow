...
// 🧠 FIX: Expresión regular que detecta 2 o más guiones bajos seguidos
    const partes = frase.split(/_{2,}/);
    
    // Por si la IA hace cosas raras, aseguramos que siempre haya un antes y un después
    const textoAntes = partes[0] || "";
    const textoDespues = partes.length > 1 ? partes.slice(1).join(" ") : "";
        ...
        <form onSubmit={handleSubmit} className="flex flex-col items-center gap-8 w-full animate-fade-in">
            ...
            <div className="text-2xl md:text-3xl font-bold text-white leading-loose text-center max-w-3xl">
                ...
                <input
                    ...
                    placeholder="..."
                    style={{ width: `${Math.max(150, valor.length * 15)}px` }}
                />
                {textoDespues}
            </div>
